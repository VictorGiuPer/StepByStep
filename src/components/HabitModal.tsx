...
function initialInput(habit: Habit | null, categoryId: string): HabitInput {
  return habit ? {
    ...
    scope: habit.scope, frequency: habit.weekly_target ? 'flexible_weekly' : habit.frequency, customDays: habit.custom_days ?? [], size: habit.size, archived: habit.archived, weeklyTarget: habit.weekly_target ?? 3,
  } : { name: '', icon: 'Sparkles', categoryId, type: 'build', scope: 'personal', frequency: 'daily', customDays: [1, 3, 5], size: 'small', archived: false, weeklyTarget: 3 }
}
    ...
    <div className={`grid gap-6 ${habit ? 'lg:grid-cols-[1.05fr_.95fr]' : ''}`}>
      <form className="rounded-[26px] bg-white p-5 shadow-soft" onSubmit={submit}>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {form.frequency === 'custom_days' && <fieldset className="sm:col-span-2"><legend className="mb-2 flex items-center justify-between text-xs font-extrabold uppercase tracking-[0.09em] text-ink/55"><span>Choose your days</span><span className="normal-case tracking-normal text-action">{form.customDays.length} {form.customDays.length === 1 ? 'time' : 'times'} per week</span></legend><p className="mb-2 text-xs leading-5 text-ink/45">Select the days this habit fits. Change them anytime without losing the habit itself.</p><div className="grid grid-cols-7 gap-1.5">{weekdays.map((day) => <label key={day.value} className={`cursor-pointer rounded-xl px-1 py-2 text-center text-xs font-black ${form.customDays.includes(day.value) ? 'bg-action text-white' : 'bg-app-bg text-ink/40'}`}><input className="sr-only" type="checkbox" disabled={!canEdit} checked={form.customDays.includes(day.value)} onChange={() => set('customDays', form.customDays.includes(day.value) ? form.customDays.filter((value) => value !== day.value) : [...form.customDays, day.value].sort())} />{day.label}</label>)}</div></fieldset>}

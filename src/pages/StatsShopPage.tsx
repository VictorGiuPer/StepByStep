...
function PendingDecision({ item, snapshot, userId, onPreviewDecision }: { item: Redemption; snapshot: AppSnapshot; userId: string; onPreviewDecision?: (redemptionId: string, decision: 'confirmed' | 'declined', reason?: string) => void }) {
  const queryClient = useQueryClient()
  const [reason, setReason] = useState('')
  const [error, setError] = useState('')
  const mutation = useMutation({ mutationFn: (decision: 'confirmed' | 'declined') => onPreviewDecision ? Promise.resolve(onPreviewDecision(item.id, decision, reason)).then(() => item) : decideRedemption(supabase, item.id, decision, reason), onSuccess: async () => queryClient.invalidateQueries({ queryKey: ['snapshot', userId] }), onError: (value) => setError((value as Error).message) })
...
export function StatsShopPage({ snapshot, userId, onPreviewRequest, onPreviewDecision }: { snapshot: AppSnapshot; userId: string; onPreviewRequest?: (rewardId: string) => void; onPreviewDecision?: (redemptionId: string, decision: 'confirmed' | 'declined', reason?: string) => void }) {
  const queryClient = useQueryClient()
  const today = dateKeyInTimeZone(snapshot.settings.timezone)
  const [range, setRange] = useState<Range>('30')
  const [editing, setEditing] = useState<Reward | null>(null)
  const [error, setError] = useState('')
  const request = useMutation({ mutationFn: (rewardId: string) => onPreviewRequest ? Promise.resolve(onPreviewRequest(rewardId)).then(() => ({ id: 'preview-request' } as Redemption)) : requestRedemption(supabase, rewardId), onSuccess: async () => queryClient.invalidateQueries({ queryKey: ['snapshot', userId] }), onError: (reason) => setError((reason as Error).message) })
  const archive = useMutation({ mutationFn: async (reward: Reward) => { if (isDemoMode) return; const { error: archiveError } = await supabase.from('rewards').update({ archived: true }).eq('id', reward.id); if (archiveError) throw friendlyError(archiveError) }, onSuccess: async () => queryClient.invalidateQueries({ queryKey: ['snapshot', userId] }), onError: (reason) => setError((reason as Error).message) })
